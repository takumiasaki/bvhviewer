...
| F03 | 3D 描画・色分け・表示切替 | `BvhSkeletonItem` が `jointColor` / `boneColor` / `colorsLinked` / `visible` / リストモデルを公開。色・Offset は Active Scene パネルで即時反映 |
...
`visible` / `color` / `displayName` は内部 `Bvh3DModel` へ委譲し、変更シグナルを転送する。
    ...
    delegate: Node {
        ...
        Repeater3D {
            ...
            Model {
                ...
                materials: [ PrincipledMaterial { baseColor: skeleton.color } ]
            }
        }
        ...
        Repeater3D {
            ...
            Model {
                ...
                scale: Qt.vector3d(0.2, model.length, 0.2)
                materials: [ PrincipledMaterial { baseColor: skeleton.color } ]
            }
        }
    }
