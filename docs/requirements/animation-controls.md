...
| v1.0.3 | 2026/06/16 | 実装状況タグ追加。将来拡張 ID を backlog と整合 |
...
要件 ID・管理は [backlog.md](./backlog.md) §6.4 および [requirements.md](./requirements.md) §6 を参照。

- ループ再生 ON/OFF
- **F10:** 再生速度（0.25x, 0.5x, 2x 等）
- **F11-3:** スケルトン個別の再生／停止
- タイムライン上のキーフレームマーカー
