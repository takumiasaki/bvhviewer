...
| :--- | :--- |
| **方針** | 読込順にパレット色を循環割当 |
| **初期状態** | ジョイント色 = ボーン色（`colorsLinked = true`） |
...
| 項目 | 仕様 |
| :--- | :--- |
| Joint Choose color… | ColorDialog → `jointColor` 即反映 |
| Same color for joints and bones | 即反映。ON 時は Bone tone 行を **disabled**（非表示にせずパネル高さ固定） |
| Bone tone スライダー | linked OFF 時のみ **enabled**。-100〜+100。操作即反映 |
| Bone Choose color… | linked OFF 時のみ **enabled** |
